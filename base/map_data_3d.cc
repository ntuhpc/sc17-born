...
bool map_data::adjoint(bool add, std::shared_ptr<my_vector> model,
  ...
  for (int i3 = 0; i3 < n3; i3++) {
    for (int i2 = 0; i2 < n2; i2++) {
      for (int it = 0; it < nt; it++) {
        m->vals[ntb * i2 + it + 4 + i3 * ntb * n2] +=
            d->vals[it + i2 * nt + i3 * nt * n2];
      }
    }
  }
}

bool map_data::forward(bool add, std::shared_ptr<my_vector> model,
                       std::shared_ptr<my_vector> data, int iter) {
  std::shared_ptr<float_3d> d = std::dynamic_pointer_cast<float_3d>(data);
  std::shared_ptr<float_3d> m = std::dynamic_pointer_cast<float_3d>(model);
  if (!add) d->zero();
