...
class col_op : public my_operator {
 public:
  ...
  void create_super();
  virtual bool forward(bool add, std::shared_ptr<my_vector> model,
                       std::shared_ptr<my_vector> data);
  virtual bool adjoint(bool add, std::shared_ptr<my_vector> model,
                       std::shared_ptr<my_vector> data);

  virtual std::shared_ptr<my_vector> range_vec(std::shared_ptr<my_vector> v1,
                                               std::shared_ptr<my_vector> v2);

  virtual void set_scale(double sc) { ops[0]->scale = ops[0]->scale * sc; }
...
class row_op : public my_operator {
 public:
  ...
  row_op(std::shared_ptr<my_operator> op) {
    std::vector<std::shared_ptr<my_operator>> os;
    os.push_back(op);
    chain_init(os);
  }
  ...
  row_op(std::shared_ptr<my_operator> op1, std::shared_ptr<my_operator> op2,
         std::shared_ptr<my_operator> op3) {
    std::vector<std::shared_ptr<my_operator>> os;
    os.push_back(op1);
    os.push_back(op2);
    os.push_back(op3);
  ...
  row_op(std::vector<std::shared_ptr<my_operator>> os) { chain_init(os); }
