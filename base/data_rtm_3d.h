...
class data_rtm_3d : public wavefield_insert_3d {
 public:
  ...
  void get_source_func(std::shared_ptr<hypercube_float> domain, int ishot,
                       std::vector<float> &s_x, std::vector<float> &s_y,
                       std::vector<float> &s_z, int nsinc, int nts,
                       std::shared_ptr<hypercube_float> time);
  // void get_source_func_encode(hypercube_float *domain, int ishot, bool
  // encode, int *rvec, float *s_z, float *s_x, float *s_y, int nsinc,  int nts,
  // hypercube_float *time);
  void set_source_file(std::shared_ptr<oc_float> pntr) { myf = pntr; }

  int nshotsx() { return getAxis(4).n; }
  int nshotsy() { return getAxis(5).n; }
  int nshots() { return getAxis(4).n * getAxis(5).n; }
