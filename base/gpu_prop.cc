...
void gpuProp::transferSincTableD(int nsinc, int jtd, std::vector<std::vector<float> >& table){
...
void gpuProp::transferSourceFunc(int npts,int nt_big,std::vector<int>& locs, float *vals){
...
void gpuProp::transferReceiverFunc(int nx, int ny, int nt, std::vector<int>& locs,
	float *rec){
...
void gpuProp::transferSincTableS(int nsinc, int jts, std::vector<std::vector<float> >& table){
	std::vector<float*> table_ptrs(table.size());
	for (size_t i = 0; i < table.size(); ++i)
		table_ptrs.at(i) = table.at(i).data();
