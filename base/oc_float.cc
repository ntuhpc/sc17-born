...
oc_float::oc_float(std::shared_ptr<SEP::genericIO> io, std::string tag,
                   std::vector<SEP::axis> axes, bool alloc) {
  _io = io;
...
void oc_float::zeroFile() { set_val(0.); }
oc_float::oc_float(std::shared_ptr<SEP::genericIO> io, std::string tag,
...
void oc_float::allocate() { zeroFile(); }
...
void oc_float::info(char *str, int level) {
  ...
  if (1 == 3) {
    ...
    std::shared_ptr<SEP::hypercube> hyper;
    for (int i = 0; i < hyper->getNdim(); i++)
      fprintf(stderr, "    n%d=%d", i + 1, getAxis(i + 1).n);
    fprintf(stderr, "\n       N=%d min(%d)=%g max(%d)=%g RMS=%g   \n",
            (int)(hyper->getN123()), imin, mymin, imax, mymax,
            sqrt(sm) / (1.0 * hyper->getN123()));
    long long print;
    if (level != 0) {
      if (level < 1)
        print = getHyper()->getN123();
      else
        print = MIN(level, BUFSZ);
      int ic = 0;
...
void oc_float::random() {
  ...
  while (ndone != ndo) {
    long long nbuf = std::min(ndo - ndone, nbuf);
    _file->readFloatStream(buf, nbuf);
    for (long long i = 0; i < nbuf; i++)
      buf[i] = (float)rand() / (float)RAND_MAX - .5;
    _file->seekTo(-nbuf * 4, 1);
    _file->writeFloatStream(buf, nbuf);
    ndone += nbuf;
  }
...
void oc_float::scale_add(double sc1, std::shared_ptr<my_vector> v1, double sc2,
...
void oc_float::scale_add(const double sc1, std::shared_ptr<my_vector> other,
                         const double sc2) {
  check_same(other);
