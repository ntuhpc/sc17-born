...
class source_func : public wavefield_insert_3d {
 public:
  ...
  void set_source_file(std::string tg) { tag = tg; }
  ~source_func() {}
  ...
  void set_compute_size(std::shared_ptr<SEP::hypercube> dom, float aper,
                        int nbt, int nb, int nby, int fat, int blocksize);
  ...
  virtual void get_source_func(std::shared_ptr<hypercube_float> domain,
                               int ishot, int nts, std::vector<int> &ilocs,
...
class wavefield_source_func : public source_func {
 public:
  ...
  virtual int get_points(bool e);
  void set_sources_depth(float s_z) { sz.push_back(s_z); }
  virtual void get_source_func(std::shared_ptr<hypercube_float> domain,
                               int ishot, int nts, std::vector<int> &ilocs,
                               std::vector<float> &vals);
