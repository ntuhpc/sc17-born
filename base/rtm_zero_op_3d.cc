...
rtm_zero_op::rtm_zero_op(std::shared_ptr<SEP::paramObj> par,
                         ...
                         std::shared_ptr<data_rtm_3d> dat,
                         std::shared_ptr<image_rtm_3d> img, float ap, bool v,
                         bool enc, std::vector<int> r_vec, bool do_src,
                         bool redo_src) {
  std::vector<float> ds;
  data = dat;
  setProp(prop);
...
void rtm_zero_op::create_source_fields() {
  ...
  for (int ishot = 0; ishot < nshots; ishot++) {
    ...
    if (verb)
      fprintf(stderr, "Forward 1propagating shot %d of %d \n", ishot, nshots);

    _prop->transferVelFunc1(nx, ny, nz, vloc->vals);
    time_t startwtimes, endwtimes;
    startwtimes = time(&startwtimes);
    if (verb)
      fprintf(stderr, "Forward2 propagating shot %d of %d \n", ishot, nshots);

    _prop->sourceProp(nx, ny, nz, false, true, src_p0->vals, src_p1->vals, jts,
                      npts, nt);
    if (verb)
      fprintf(stderr, "Forward3 propagating shot %d of %d \n", ishot, nshots);
...
bool rtm_zero_op::forward(bool add, std::shared_ptr<my_vector> mvec,
  ...
  std::shared_ptr<image_rtm_3d> m =
      std::dynamic_pointer_cast<image_rtm_3d>(mvec);
  ...
  std::shared_ptr<float_3d> shot(
      new float_3d(data->getAxis(1), data->getAxis(2), data->getAxis(3)));
  std::shared_ptr<float_3d> shot_deriv(
      new float_3d(data->getAxis(1), data->getAxis(2), data->getAxis(3)));
  ...
  for (int ishot = 0; ishot < nshots; ishot++) {
    ...
    std::shared_ptr<hypercube_float> vrand =
        vel->rand_subspace(seed, nbound, nbound, nbound, dt, src_p0);
    fprintf(stderr, "before none \n");

    std::shared_ptr<hypercube_float> vnone =
        vel->rand_subspace(seed, 0, 0, 0, dt, src_p0);
    ...
    map_data mapit(npts, s_x, s_y, s_z, locs, src_p0, rec_func, shot_deriv,
                   nt_big);
    ...
    _prop->rtmForward(ad1.n, ad2.n, ad3.n, jtd, img->vals, rec_func->vals,
                      npts_s, nt, nt_big, rec_nx, rec_ny);
...
bool rtm_zero_op::adjoint(bool add, std::shared_ptr<my_vector> mvec,
  ...
  std::vector<float> vals(npts_s * nt_big, 0.), s_x(npts, 0.), s_z(npts, 0.),
      s_y(npts, 0.);
  ...
  for (int ishot = 0; ishot < nshots; ishot++) {
    if (verb)
      fprintf(stderr, "Adjoint shot %d of %d \n", ishot, data->nshots());
    std::shared_ptr<hypercube_float> src_p0 = source->create_domain(ishot);
    std::shared_ptr<hypercube_float> src_p1 = src_p0->clone();
    SEP::axis ad1 = src_p0->getAxis(1);
    SEP::axis ad2 = src_p0->getAxis(2);
    ...
    _prop->rtmAdjoint(ad1.n, ad2.n, ad3.n, jtd, src_p1->vals, src_p0->vals,
                      img->vals, npts_s, nt);
