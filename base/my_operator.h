...
class my_operator {
 public:
  ...
  bool check_domain_range(std::shared_ptr<my_vector> dom,
                          std::shared_ptr<my_vector> range);
  virtual bool forward(bool add, std::shared_ptr<my_vector> model,
                       std::shared_ptr<my_vector> data, int iter = 1) {
    return false;
  }
  ...
  virtual void hessian(std::shared_ptr<my_vector> in,
                       std::shared_ptr<my_vector> out);
