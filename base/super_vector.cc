...
super_vector::super_vector(std::shared_ptr<my_vector> v1,
                           std::shared_ptr<my_vector> v2, bool sp) {
...
super_vector::super_vector(std::shared_ptr<my_vector> v1,
                           ...
                           std::shared_ptr<my_vector> v3, bool sp) {
  super_init("super_vector");
  just_space = sp;
...
super_vector::super_vector(std::vector<std::shared_ptr<my_vector>> vs,
                           bool sp) {
  super_init("super_vector");
  just_space = sp;
  ...
  if (just_space) {
    for (int i = 0; i < (int)vs.size(); i++)
      vecs.push_back(vs[i]->clone_space());
  } else {
...
void super_vector::scale_add(const double mes, std::shared_ptr<my_vector> vec,
                             const double other) {
  if (!check_match(vec)) fprintf(stderr, "vectors don't match scale_add\n");
  std::shared_ptr<super_vector> v =
      std::dynamic_pointer_cast<super_vector>(vec);
  for (int i = 0; i < (int)vecs.size(); i++)
    vecs[i]->scale_add(mes, v->vecs[i], other);
}
...
void super_vector::add(std::shared_ptr<my_vector> vec) {
  if (!check_match(vec)) fprintf(stderr, "vectors don't match add\n");
  std::shared_ptr<super_vector> v =
      std::dynamic_pointer_cast<super_vector>(vec);
  for (int i = 0; i < (int)vecs.size(); i++) vecs[i]->add(v->vecs[i]);
}
double super_vector::dot(std::shared_ptr<my_vector> vec) {
  if (!check_match(vec)) fprintf(stderr, "vectors don't match dot\n");
  std::shared_ptr<super_vector> v =
      std::dynamic_pointer_cast<super_vector>(vec);
  double dd = 0., xx;
  for (int i = 0; i < (int)vecs.size(); i++) {
...
void super_vector::inverse_hessian(std::shared_ptr<my_vector> vec) {
  ...
  std::shared_ptr<super_vector> v =
      std::dynamic_pointer_cast<super_vector>(vec);
  for (int i = 0; i < (int)vecs.size(); i++)
    vecs[i]->inverse_hessian(v->vecs[i]);
}
