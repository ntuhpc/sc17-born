...
col_op::col_op(std::shared_ptr<my_operator> op1,
               std::shared_ptr<my_operator> op2) {
  ops.push_back(op1);
  ops.push_back(op2);
  create_super();
}
...
bool col_op::forward(bool add, std::shared_ptr<my_vector> model,
                     std::shared_ptr<my_vector> data) {
  ...
  std::shared_ptr<super_vector> d =
      std::dynamic_pointer_cast<super_vector>(data);
...
bool col_op::adjoint(bool add, std::shared_ptr<my_vector> model,
                     std::shared_ptr<my_vector> data) {
  if (!check_domain_range(model, data)) {
...
diag_op::diag_op(std::shared_ptr<my_operator> op1,
                 std::shared_ptr<my_operator> op2) {
  ops.push_back(op1);
  ops.push_back(op2);
  create_super();
...
bool diag_op::forward(bool add, std::shared_ptr<my_vector> model,
                      std::shared_ptr<my_vector> data) {
  if (!check_domain_range(model, data)) {
    fprintf(stderr, "diag op match failed(3)\n");
    return false;
  }
  std::shared_ptr<super_vector> m =
      std::dynamic_pointer_cast<super_vector>(model);
  std::shared_ptr<super_vector> d =
      std::dynamic_pointer_cast<super_vector>(data);
  for (int i = 0; i < ops.size(); i++) {
...
bool diag_op::adjoint(bool add, std::shared_ptr<my_vector> model,
                      std::shared_ptr<my_vector> data) {
  if (!check_domain_range(model, data)) {
    fprintf(stderr, "diag op match failed(3)\n");
    return false;
...
bool row_op::forward(bool add, std::shared_ptr<my_vector> model,
                     std::shared_ptr<my_vector> data) {
  if (!check_domain_range(model, data)) {
...
bool row_op::adjoint(bool add, std::shared_ptr<my_vector> model,
                     std::shared_ptr<my_vector> data) {
  if (!check_domain_range(model, data)) {
...
std::shared_ptr<my_vector> col_op::range_vec(std::shared_ptr<my_vector> v1,
...
std::shared_ptr<my_vector> diag_op::domain_vec(std::shared_ptr<my_vector> v1,
                                               std::shared_ptr<my_vector> v2) {
  std::shared_ptr<super_vector> vec(new super_vector(v1, v2));
...
std::shared_ptr<my_vector> diag_op::range_vec(std::shared_ptr<my_vector> v1,
                                              std::shared_ptr<my_vector> v2) {
  std::shared_ptr<super_vector> vec(new super_vector(v1, v2));
  if (!vec->check_match(range)) {
    fprintf(stderr, "range vector doesn't match suplied vecs");
...
bool row_op::check_valid_chain() {
  for (int i = 0; i < (int)ops.size() - 1; i++) {
    if (!ops[i]->range->check_match(ops[i + 1]->domain)) {
      fprintf(stderr,
              "domain and range don't match when creating super operator (%d "
              "%d) \n",
              i, i + 1);
      return false;
    }
...
void row_op::add_op(std::shared_ptr<my_operator> op) {
  ops.push_back(op);
  if (!check_valid_chain()) fprintf(stderr, "trouble adding operator\n");
}
