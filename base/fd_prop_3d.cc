...
bool fd_prop::calc_stability(std::vector<float> &ds, int n) {
  ...
  for (int i = 0; i < (int)ds.size(); i++)
    d = ds[i] / dt - (int)(ds[i] / dt + .001);

  /*
          if(fabs(d) >.01) {
                  param->error(std::string("sampling match problem
     ")+std::to_string(d)+ std::string("
     ")+std::to_string(dt)+std::string("\n"));
          }
   */
...
void fd_prop::set_fd_basics(std::shared_ptr<SEP::paramObj> par,
                            std::shared_ptr<source_func> source_func, float ap,
                            bool v) {
  set_verb(v);
  aper = ap;
  param = par;
  ...
  source->set_compute_size(vel->_vel, aper, nbound, nbound, nbound, fat,
                           blocksize);
  nx = source->x_points();
  nz = source->z_points();
  ny = source->y_points();
  _prop->createSpace(vel->getAxis(1).d, vel->getAxis(2).d, vel->getAxis(3).d,
                     bc_a, bc_b, bc_b_y, nx, ny, nz);
}
