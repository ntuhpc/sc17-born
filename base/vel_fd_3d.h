...
class vel_fd_3d {
 public:
   ...
   */
  vel_fd_3d(std::shared_ptr<SEP::genericIO> io, std::string tag);
  /** constructor
     \param io class handling parameters and iO
   ...
   */
  vel_fd_3d(std::shared_ptr<SEP::genericIO> io, std::string tagV,
            std::string tagD);
  /**
   ...
   */
  std::shared_ptr<hypercube_float> rand_subspace(
      const int irand, const int nrandt, const int nrandblr, const int randblr2,
  ...
  void set_old() { _old = true; }
  void set_mid() { _mid = true; }
  bool has_dens() {
    if (!_dens) return false;
    return true;
  }
  float max_vel() { return (float)_vel->my_max(); }
  float min_vel() { return (float)_vel->my_min(); }
  SEP::axis getAxis(int iax) { return _vel->getAxis(iax); }
  void set_zero() { _zero = true; }
  ~vel_fd_3d() { fprintf(stderr, "in delete 1 \n"); }
  /** io being used*/
