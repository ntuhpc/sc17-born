...
void source_func::set_compute_size(std::shared_ptr<SEP::hypercube> dom,
                                   float ap, int nbt, int nb, int nby, int fat,
                                   int blocksize) {
  ax = dom->getAxis(1);
  ay = dom->getAxis(2);
  az = dom->getAxis(3);
  ...
  // fprintf(stderr,"  Remainders are %d %d %d from %d %d %d. Padding z with %d
  // + %d ; %d\n",rem_x,rem_y,rem_z,ax.n,ay.n,az.n,nbt,nb,nboundt);

  ay.n += rem_y;
  ax.n += rem_x;
  az.n += rem_z;
  // fprintf(stderr," Set comp %d %d %d , %f %f
...
void wavelet_source_func::get_source_func(
    std::shared_ptr<hypercube_float> domain, int ishot, int nts,
    std::vector<int> &locs, std::vector<float> &time) {
...
int wavelet_source_func::get_points(bool e) { return 9; }
wavelet_source_func::wavelet_source_func(std::shared_ptr<SEP::genericIO> io,
                                         std::string tag) {
  _io = io;
  tagInit(tag);
...
void wavelet_source_func::set_sources_axes(float s_z, SEP::axis src_axis1,
                                           SEP::axis src_axis2) {
...
wavefield_source_func::wavefield_source_func(std::shared_ptr<SEP::genericIO> io,
                                             std::string tag) {
  _io = io;
  tagInit(tag);
  std::shared_ptr<hypercube_float> tmp(new hypercube_float(getHyper()));
  ...
  for (long long i = 0; i < ntraces; i++) {
    for (long long it = 0; it < nt - 1; it++) {
      wavefield->vals[it + i * nt] =
          (tmp->vals[it + 1 + i * nt] - tmp->vals[it + i * nt]) /
          tmp->getAxis(1).d;
    }
  }
...
void wavefield_source_func::get_source_func(
    std::shared_ptr<hypercube_float> domain, int ishot, int nts,
    std::vector<int> &locs, std::vector<float> &time) {
  SEP::axis a1 = domain->getAxis(1);
  SEP::axis a2 = domain->getAxis(2);
  ...
  int ipos = 0;
  for (long long i = 0;
       i < (long long)getAxis(3).n * (long long)getAxis(2).n * (nts); i++)
    time[i] = 0;
