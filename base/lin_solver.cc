...
simple_solver::simple_solver(step *s, std::shared_ptr<my_vector> data,
                             std::shared_ptr<my_operator> op,
                             std::shared_ptr<my_operator> wt,
...
prec_solver::prec_solver(step *s, std::shared_ptr<my_vector> data,
                         std::shared_ptr<my_operator> op,
                         std::shared_ptr<my_operator> prec, float eps,
                         std::shared_ptr<my_operator> wt,
                         std::shared_ptr<my_vector> m0) {
  init_solve(s);
  have_op = true;
...
reg_solver::reg_solver(step *s, std::shared_ptr<my_vector> data,
                       std::shared_ptr<my_operator> op,
                       std::shared_ptr<my_operator> reg, float eps,
                       std::shared_ptr<my_operator> wt,
                       std::shared_ptr<my_vector> m0) {
  init_solve(s);
  have_op = true;
...
void lin_solver::create_wt_op(std::shared_ptr<my_operator> op,
                              std::shared_ptr<my_operator> wt) {
  if (wt != 0) {
    wt_op.reset(new row_op(op, wt));
    have_wt = true;
  } else {
...
std::shared_ptr<my_vector> lin_solver::init_rr(std::shared_ptr<my_vector> data,
                                               std::shared_ptr<my_operator> op,
                                               std::shared_ptr<my_operator> wt,
                                               std::shared_ptr<my_vector> m0) {
...
bool lin_solver::solve(int niter) {
  ...
  for (int iter = 0; iter < niter; iter++) {
    ...
    fprintf(stderr, "Iteration %d, wall clock time = %f \n", iter,
            difftime(enditime, startitime));
  }
  return true;
}
std::shared_ptr<my_vector> lin_solver::return_model() { return m->clone_vec(); }
void lin_solver::update_model(std::shared_ptr<my_vector> mod) { mod->add(m); }
