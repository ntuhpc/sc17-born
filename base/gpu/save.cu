...
extern "C" __global__ void new_src_inject_kernel(int it, int isinc, float *p) {
  ...
  p[srcgeom_gpu0[ix]] += dir_gpu * (sinc_s_table[isinc * nsinc_gpu] *
                                        source_gpu0[ntrace_gpu * ix + it] +
...
void rtm_forward(int n1, int n2, int n3, int jt, float *img, float *dat,
  ...
  for (int i = 0; i < n_gpus; i++) {
    ...
    cudaMemcpy(
        img_gpu[i] /*+radius*n1*n2*/, img + i * n1 * n2 * (n3 - 2 * radius),
  ...
  for (int it = 0; it < nt; it++) {
    ...
    //    kernel_exec(damp_kernel<<<dimGrid, dimBlock>>>(nblocksz,data_p0,
    //    data_p1));
    for (int i = 0; i < n_gpus; i++) {
      cudaSetDevice(device[i]);
      if (i > 0) {
        wave_kernel_adj<<<dimGrid, dimBlock, 0, stream_halo[i]>>>(
            data_p0[i] + offset_cmp_h1, data_p1[i] + offset_cmp_h1,
            data_p0[i] + offset_cmp_h1, velocity[i] + offset_cmp_h1, radius,
            2 * radius);
        wave_kernel<<<dimGrid, dimBlock, 0, stream_halo[i]>>>(
            src_p0[i] + offset_cmp_h1, src_p1[i] + offset_cmp_h1,
            src_p0[i] + offset_cmp_h1, velocity[i] + offset_cmp_h1, radius,
      ...
      if (i < n_gpus - 1) {
        wave_kernel_adj<<<dimGrid, dimBlock, 0, stream_halo[i]>>>(
            ...
            data_p0[i] + offset_cmp_h2, velocity[i] + offset_cmp_h2,
            (n3 - radius) - radius, n3 - radius);
        wave_kernel<<<dimGrid, dimBlock, 0, stream_halo[i]>>>(
            src_p0[i] + offset_cmp_h2, src_p1[i] + offset_cmp_h2,
            src_p0[i] + offset_cmp_h2, velocity[i] + offset_cmp_h2,
    ...
    for (int i = 0; i < n_gpus; i++) {
      ...
      damp_kernel<<<dimGrid, dimBlock, 0, stream_internal[i]>>>(
          data_p0[i], data_p1[i], start3[i], end3[i], i, n_gpus);
      damp_kernel<<<dimGrid, dimBlock, 0, stream_internal[i]>>>(
          data_p0[i], data_p1[i], start3[i], end3[i], i, n_gpus);
      damp_kernel<<<dimGrid, dimBlock, 0, stream_internal[i]>>>(
          src_p0[i], src_p1[i], start3[i], end3[i], i, n_gpus);

      wave_kernel_adj<<<dimGrid, dimBlock, 0, stream_internal[i]>>>(
          data_p0[i] + offset_internal[i], data_p1[i] + offset_internal[i],
          data_p0[i] + offset_internal[i], velocity[i] + offset_internal[i],
          start3[i], end3[i]);
    ...
    for (int i = 0; i < n_gpus; i++) {
      ...
      if (it % jt == 0)
        img_add_kernel<<<dimGrid, dimBlock, 0, stream_internal[i]>>>(
            img_gpu[i], data_p0[i], src_p0[i]);
      // if(it%jt==0)
    ...
    cudaSetDevice(device[0]);
    new_data_extract_kernel<<<dimGridx, dimBlock, 0, stream_internal[0]>>>(
...
void rtm_adjoint(int n1, int n2, int n3, int jt, float *p0_s_cpu,
  ...
  for (int it = nt - 1; it >= 0; it--) {
    ...
    for (int i = 0; i < n_gpus; i++) {
      ...
      if (i < n_gpus - 1) {
        wave_kernel<<<dimGrid, dimBlock, 0, stream_halo[i]>>>(
            data_p0[i] + offset_cmp_h2, data_p1[i] + offset_cmp_h2,
            data_p0[i] + offset_cmp_h2, velocity[i] + offset_cmp_h2, radius,
            2 * radius);

        if (it < nt - 1)
          wave_kernel<<<dimGrid, dimBlock, 0, stream_halo[i]>>>(
              src_p0[i] + offset_cmp_h2, src_p1[i] + offset_cmp_h2,
              src_p0[i] + offset_cmp_h2, velocity[i] + offset_cmp_h2, radius,
              2 * radius);  // THIS IS THE NAUGHTY ONE
      }
      cudaStreamQuery(stream_halo[i]);
    }
    ...
    /*if(it==(nt-200)){
      for(int i=0; i<n_gpus; i++){
        cudaSetDevice(device[i]);
        cudaMemcpy(snap+i*n1*n2*(n3-2*radius), data_p0[i]+radius*n1*n2,
  ...
  for (int i = 0; i < n_gpus; i++) {
    ...
    cudaMemcpy(
        img + i * n1 * n2 * (n3 - 2 * radius), img_gpu[i] /*+radius*n1*n2*/,
        n1 * n2 * (n3 /*-2*radius*/) * sizeof(float), cudaMemcpyDeviceToHost);
    // cudaMemcpy(img+i*n1*n2*(n3-2*radius), img_gpu[i]+radius*n1*n2,
    // n1*n2*(n3-2*radius)*sizeof(float), cudaMemcpyDeviceToHost);  cudaMemcpy(
    // img_gpu[i]/*+radius*n1*n2*/, img+i*n1*n2*(n3-2*radius),
    // n1*n2*(n3/*-2*radius*/)*sizeof(float), cudaMemcpyHostToDevice);
  }
...
void transfer_receiver_func(int nx, int ny, int nt, int *locs, float *vals) {
  ...
  cudaMemcpy(datageom_gpu, locs, nx * ny * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(data_gpu, vals,
             (long long)nt * (long long)nx * (long long)ny * sizeof(float),
             cudaMemcpyHostToDevice);
...
void create_gpu_space(float d1, float d2, float d3, float bc_a, float bc_b,
  ...
  float coeffs_cpu[COEFFS_SIZE] =
      get_coeffs((double)d1, (double)d2, (double)d3);
