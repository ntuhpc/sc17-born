...
#define hloc(z, x, y) \
  (z) + (x)*nz +      \
      (y)*nz *        \
          nx /* host mem. z,x,y-coordinates to linear address converter */  // ok. add y to these location addresses
#define dloc(z, x, y) \
  (z) + (x)*n1gpu +   \
      (y)*n1gpu *     \
          n2gpu /* device mem. z,x,y-coordinate to linear address converter */
...
#define IK1_BLKSIZE \
  16 /* imaging kernel 1 & 2 variable */  // leave these for now....
...
__device__ int min4(int v1, int v2, int v3, int v4) {
  return min2(min2(v1, v2), min2(v3, v4));
}
__device__ int min5(int v1, int v2, int v3, int v4, int v5) {
  return min2(min2(min2(v1, v2), min2(v3, v4)), v5);
}
