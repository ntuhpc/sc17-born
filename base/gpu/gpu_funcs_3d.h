...
void rtm_adjoint(int n1, int n2, int n3, int j, float *p0_s_cpu,
                 float *p1_s_cpu, float *img, int npts, int nt);  // done
...
void transfer_receiver_func(
    int nx, int ny, int nt, int *locs,
    float *
        vals /*, float *s_z, float *s_x, float *s_y*/);  //(think) nothing to do
...
void create_gpu_space(float d1, float d2, float d3, float bc_a, float bc_b,
                      float bc_b_y, int nx, int ny, int nz);  // done
};
#endif
