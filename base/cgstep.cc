...
#include <math.h>
#include <iostream>
void cgstep::alloc_step(std::shared_ptr<my_vector> mod,
                        std::shared_ptr<my_vector> dat) {
  s = mod->clone_vec();
  ss = dat->clone_vec();

  forget_it();
}
bool cgstep::step_it(int ith, std::shared_ptr<my_vector> m,
                     std::shared_ptr<my_vector> g,
                     std::shared_ptr<my_vector> rr,
                     std::shared_ptr<my_vector> gg, double *vv) {
  double alfa, beta;
  if (forget) {
    beta = 0;
