...
void data_rtm_3d::add_data(int ishot, std::shared_ptr<hypercube_float> dat) {
  ...
  int n2 = getAxis(2).n;
  int n3 = getAxis(3).n;
  _file->seekTo((long long)(getAxis(2).n * ishot * getAxis(3).n) *
                    (long long)(getAxis(1).n * 4),
                0);

  std::shared_ptr<hypercube_float> tmp = dat->clone();
  tmp->scale(0.);
...
void data_rtm_3d::get_source_func(std::shared_ptr<hypercube_float> domain,
                                  int ishot, std::vector<float> &s_x,
                                  std::vector<float> &s_y,
                                  std::vector<float> &s_z, int nsinc, int nts,
                                  std::shared_ptr<hypercube_float> time) {
  ...
  _file->seekTo(ishot * (long long)(getAxis(3).n * getAxis(2).n) *
                    (long long)(getAxis(1).n * 4),
                0);
  _file->readFloatStream(time->vals,
                         getAxis(1).n * getAxis(2).n * getAxis(3).n);
...
/*void data_rtm_3d::get_source_func_encode(hypercube_float *domain, int ishot,
   bool encode, int *rvec, float *s_z, float *s_x, float *s_y, int nsinc,  int
   nts,hypercube_float *time){
   ...
   for(int is=0; is<ns; is++){
    ...
    for(int k=0; k < (int) tmp->get_n123(); k++) time->vals[k] +=
   rvec[is]*tmp->vals[k]/ns; fprintf(stderr,"    Data summing, %d %d
   \n",is,rvec[is]);


   }
   fprintf(stderr,"Shots summed \n");
    // Set up the regular source geometry RIGHT NOW THIS IS THE SAME FOR ALL
   SHOTS for(int i3=0; i3< (getAxis(3).n); i3++){ for(int i2=0; i2<
   getAxis(2).n; i2++, i++){ s_z[i]=(sz-a1.o)/a1.d;
