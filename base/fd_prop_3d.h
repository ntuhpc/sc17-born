...
class fd_prop {
 public:
  ...
  void setProp(std::shared_ptr<baseProp> prop) { _prop = prop; };
  void set_vel(std::shared_ptr<vel_fd_3d> v) { vel = v; }

  void set_verb(bool v) { verb = v; }
  void set_bounds(int n1, int n2, int n3) {
    nboundt = n1;
    nbound = n2, nbound_y = n3;
  }
  void set_fd_basics(std::shared_ptr<SEP::paramObj> par,
