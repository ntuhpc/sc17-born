...
void hypercube_float::info(char *str, int level) {
  ...
  for (int i = 0; i < getNdim(); i++)
    fprintf(stderr, "    n%d=%d", i + 1, getAxis(i + 1).n);
  for (int i = 0; i < getNdim(); i++)
    fprintf(stderr, "    o%d=%f", i + 1, getAxis(i + 1).o);
  for (int i = 0; i < getNdim(); i++)
    fprintf(stderr, "    d%d=%f", i + 1, getAxis(i + 1).d);
  fprintf(stderr, "\n       N=%d min(%d)=%g max(%d)=%g RMS=%g   \n",
          (int)getN123(), imin, mymin, imax, mymax,
          sqrt(sm) / (1.0 * getN123()));
  long long print;
  if (level != 0) {
    if (level < 1)
...
double hypercube_float::dot(std::shared_ptr<my_vector> other) {
  double ret = 0;
  check_same(other);
  std::shared_ptr<hypercube_float> o =
      std::dynamic_pointer_cast<hypercube_float>(other);
...
void hypercube_float::scale_add(double sc1, std::shared_ptr<my_vector> v1,
  ...
  check_same(v1);
  check_same(v2);
  std::shared_ptr<hypercube_float> v_1 =
      std::dynamic_pointer_cast<hypercube_float>(v1);
  std::shared_ptr<hypercube_float> v_2 =
      std::dynamic_pointer_cast<hypercube_float>(v2);
  for (long long i = 0; i < getN123(); i++) {
...
void hypercube_float::take_min(std::shared_ptr<my_vector> other,
                               std::shared_ptr<my_vector> change) {
  check_same(other);
  ...
  if (!change) {
    check_same(change);
    std::shared_ptr<hypercube_float> c =
        std::dynamic_pointer_cast<hypercube_float>(change);
  }
  for (long long i = 0; i < getN123(); i++) {
    if (o->vals[i] < vals[i]) {
      vals[i] = o->vals[i];
...
void hypercube_float::take_max(std::shared_ptr<my_vector> other,
                               std::shared_ptr<my_vector> change) {
  check_same(other);
  std::shared_ptr<hypercube_float> c;
  std::shared_ptr<hypercube_float> o =
...
void hypercube_float::scale_add(const double scale_me,
                                std::shared_ptr<my_vector> other,
                                const double scale_other) {
  check_same(other);
  std::shared_ptr<hypercube_float> o =
      std::dynamic_pointer_cast<hypercube_float>(other);
  for (int i = 0; i < getN123(); i++) {
    this->vals[i] = this->vals[i] * scale_me + scale_other * o->vals[i];
  }
}
...
bool hypercube_float::check_match(std::shared_ptr<my_vector> v2) {
  ...
  std::shared_ptr<hypercube_float> h2 =
      std::dynamic_pointer_cast<hypercube_float>(v2);
