...
int main(int argc, char **argv) {
  ...
  int nx = par->getInt("nx", 400);
  float dx = par->getFloat("dx", .01);
  axes.push_back(SEP::axis(nx, 0., dx));
  int ny = par->getInt("ny", 400);
  float dy = par->getFloat("dy", .01);
  axes.push_back(SEP::axis(ny, 0., dy));
  int nz = par->getInt("nz", 400);
  float dz = par->getFloat("dz", .01);
  axes.push_back(SEP::axis(nz, 0., dz));
